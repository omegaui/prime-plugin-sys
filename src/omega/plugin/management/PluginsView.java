...
public class PluginsView extends JFrame{
	...
	public PluginsView(PluginManager pluginManager){
		super("Manage Installed Plugins");
	...
	public void init(){
		...
		titleComp = new TextComp("Manage Installed Plugins", back2, back2, glow, null);
		...
		mainPanel = new FlexPanel(null, c2, null){
		...
		listPanel.setBounds(5, 5, mainPanel.getWidth()/3 - 5, mainPanel.getHeight() - 10);
		listPanel.setArc(10, 10);
		mainPanel.add(listPanel);
		
		contentPanel = new FlexPanel(null, back1, null);
		contentPanel.setBounds(mainPanel.getWidth()/3 + 5, 5, mainPanel.getWidth() - mainPanel.getWidth()/3 - 10, mainPanel.getHeight() - 10);
		contentPanel.setArc(10, 10);
		mainPanel.add(contentPanel);
		
		listScrollPane = new JScrollPane(listContentPanel = new JPanel(null));
		listScrollPane.setBounds(5, 5, listPanel.getWidth() - 10, listPanel.getHeight() - 10);
	...
	public void moveLeft(){
		if(currentComp.screenshots.isEmpty())
			return;
		if(currentComp.pointer - 1 >= 0)
			currentComp.pointer--;
	...
	public void moveRight(){
		if(currentComp.screenshots.isEmpty())
			return;
		if(currentComp.pointer + 1 < currentComp.screenshots.size())
			currentComp.pointer++;
		screenshotComp.image = currentComp.screenshots.get(currentComp.pointer);
		screenshotComp.repaint();
	}
	...
	public void genView(){
		...
		for(Plugin plugin : pluginManager.plugins){
			LocalPluginComp comp = new LocalPluginComp(this, plugin, listScrollPane.getWidth(), 60);
		...
		listContentPanel.setPreferredSize(new Dimension(listScrollPane.getWidth(), block));
		layout();
		repaint();
	...
	public void genView(LocalPluginComp comp){
		...
		nameComp.color3 = PluginCategory.getSuitableColor(comp.plugin.getPluginCategory());
		...
		categoryComp.setText("Available Under Category of " + (comp.plugin.getPluginCategory().equals("") ? "All" : comp.plugin.getPluginCategory().toUpperCase()));
				...
				try{
					if(!comp.plugin.getScreenshots().isEmpty()){
						...
						for(int i = 0; i < comp.plugin.getScreenshots().size(); i++){
							screenshotComp.setText("Fetching Screenshots ... " + (i + 1) + " of " + comp.plugin.getScreenshots().size());
							comp.screenshots.add(ImageIO.read(comp.plugin.getScreenshots().get(i)));
						}
